...
class DBSalesInfo extends DB{
    ...
    private function getSalesinfo($salesDate, $customerID){
        ...
        salesinfo.Quantity,goods.Price*salesinfo.Quantity FROM salesinfo INNER JOIN customer ON salesinfo.CustomerID=customer.CustomerID 
        INNER JOIN goods ON salesinfo.GoodsID=goods.GoodsID WHERE salesinfo.SalesDate=? and salesinfo.CustomerID=? ORDER BY salesinfo.id
eof;
...
eof;
    ...
    public function TotalAmount($salesDate, $CustomerID){
        ...
        FROM salesinfo INNER JOIN goods ON salesinfo.GoodsID = goods.GoodsID WHERE salesinfo.SalesDate = ? AND salesinfo.CustomerID = ?
eof;
        $array = array($salesDate, $CustomerID);
        $res = parent::executeSQL($sql,$array);
    ...
    public function DeleteSlip(){
        $sql = "DELETE FROM salesinfo WHERE SalesDate=? AND CustomeiID=?";
    ...
    public function SelectSlips($salesDate){
        //日付で抽出
        $sql = <<<eof
        SELECT distinct salesinfo.SalesDate,salesinfo.CustomerID,customer.CustomerName FROM salesinfo INNER JOIN 
        customer ON salesinfo.CustomerID=customer.CustomerID WHERE salesinfo.SalesDate=? ORDER BY customer.CustomerID
eof;
