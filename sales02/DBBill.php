...
class DBBill extends DB{
    ...
    private function SelectCustomers($startDate, $endDate){
        ...
        $sql = <<<eof
        SELECT distinct salesinfo.CustomerID,customer.CustomerName FROM salesinfo INNER JOIN customer ON 
        salesinfo.CustomerID=customer.CustomerID WHERE salesinfo.SalesDate BETWEEN ? AND ? ORDER BY salesinfo.CustomerID
eof;
    ...
    public function TotalAmount($startDate, $endDate, $CustomerID){
        //請求書の合計額
        $sql = <<<eof
        SELECT sum(salesinfo.Quentity*goods.Price)
...
eof;
    ...
    private function getSalesinfo($startDate, $endDate, $CustomerID){
        $sql = <<<eof
        SELECT salesinfo.id,salesinfo.SalesDate,salesinfo.GoodsID,goods.Goodsname,goods.Price,
        salesfinfo.Quentity,(goods.Price*salesinfo.Quentity) FROM salesinfo INNER JOIN goods ON 
        salesinfo.GoodsID=goods.GoodsID WHERE salesinfo.SalesDate BETWEEN ? AND ? AND 
        salesinfo.CustomerID=? ORDER BY salesinfo.SalesDate,salesinfo.id
eof;
        $array = array($startDate, $endDate, $CustomerID);
        $res = parent::executeSQL($sql, $array);
        return $res;
    ...
    public function SelectSalesinfo($startDate, $endDate, $CustomerID){
        //$fieldCount = 7;
        $tag = "<table>\n";
        $tag .= "<tr><th>ID</th><th>日付</th><th>顧客名</th><th>商品名</th><th>単価</th><th>数量</th>
                 <th>金額</th><th></th><th></th></tr>\n";
